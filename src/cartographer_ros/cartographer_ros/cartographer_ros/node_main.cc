...
void Run() {
	...
	if (options.use_laser_scan) {
		laser_scan_subscriber = node.node_handle()->subscribe(kLaserScanTopic, kInfiniteSubscriberQueueSize,
				boost::function<void(const sensor_msgs::LaserScan::ConstPtr&)>([&](const sensor_msgs::LaserScan::ConstPtr& msg) {
	...
	if (options.use_multi_echo_laser_scan) {
		laser_scan_subscriber = node.node_handle()->subscribe(kMultiEchoLaserScanTopic, kInfiniteSubscriberQueueSize,
				boost::function<void(const sensor_msgs::MultiEchoLaserScan::ConstPtr&)>([&](const sensor_msgs::MultiEchoLaserScan::ConstPtr& msg) {
					node.map_builder_bridge()
					->sensor_bridge(trajectory_id)
	...
	if (options.num_point_clouds > 0) {
		for (int i = 0; i < options.num_point_clouds; ++i) {
			...
			point_cloud_subscribers.push_back(
					node.node_handle()->subscribe(topic, kInfiniteSubscriberQueueSize,
							boost::function<void(const sensor_msgs::PointCloud2::ConstPtr&)>([&, topic](const sensor_msgs::PointCloud2::ConstPtr& msg) {
								node.map_builder_bridge()
								->sensor_bridge(trajectory_id)
	...
	if (options.map_builder_options.use_trajectory_builder_3d()
			|| (options.map_builder_options.use_trajectory_builder_2d() && options.map_builder_options.trajectory_builder_2d_options().use_imu_data())) {
		imu_subscriber = node.node_handle()->subscribe(kImuTopic, kInfiniteSubscriberQueueSize,
				boost::function<void(const sensor_msgs::Imu::ConstPtr& msg)>([&](const sensor_msgs::Imu::ConstPtr& msg) {
					node.map_builder_bridge()
					->sensor_bridge(trajectory_id)
	...
	if (options.use_odometry) {
		odometry_subscriber = node.node_handle()->subscribe(kOdometryTopic, kInfiniteSubscriberQueueSize,
				boost::function<void(const nav_msgs::Odometry::ConstPtr&)>([&](const nav_msgs::Odometry::ConstPtr& msg) {
					node.map_builder_bridge()
					->sensor_bridge(trajectory_id)
	...
	::ros::ServiceServer finish_trajectory_server = node.node_handle()->advertiseService(kFinishTrajectoryServiceName,
			boost::function<bool(::cartographer_ros_msgs::FinishTrajectory::Request&, ::cartographer_ros_msgs::FinishTrajectory::Response&)>([&](
