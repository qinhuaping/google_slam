...
void CastRay(const Eigen::Array2i& begin, const Eigen::Array2i& end, const std::function<void(const Eigen::Array2i&)>& visitor) {
...
void CastRays(const sensor::LaserFan& laser_fan, const MapLimits& limits, const std::function<void(const Eigen::Array2i&)>& hit_visitor,
	...
	const Eigen::Array2i begin = superscaled_limits.GetXYIndexOfCellContainingPoint(laser_fan.origin.x(), laser_fan.origin.y());

	// Compute and add the end points.
	std::vector < Eigen::Array2i > ends;
	ends.reserve(laser_fan.returns.size());
	for (const Eigen::Vector3f& laser_return : laser_fan.returns) {
		ends.push_back(superscaled_limits.GetXYIndexOfCellContainingPoint(laser_return.x(), laser_return.y()));
